...
- `MOCK_DATA_ENABLED` (default: `false`; seeds test data for an existing user on startup)
- `MOCK_DATA_USER_EMAIL` (default: `testuser1@dw.com`; target user for mock seeding)
- `MOCK_DATA_RESET` (default: `false`; removes previously seeded `seed-dw-v1-*` records before seeding)
